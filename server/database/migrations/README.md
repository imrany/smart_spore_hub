...
**What it does:** Runs all pending migrations to bring your
database to the latest version.

```go
// Automatically runs in InitStorage()
database.InitStorage(connString)
...
```go
// Force to version 2
err := database.ForceMigrationVersion(2)
```
...
```bash
GOBIN=$(pwd)/bin go install github.com/imrany/gemmie/gemmie-server/cmd/migrate
```
...
```cmd
set GOBIN=./bin
go install github.com/imrany/gemmie/gemmie-server/cmd/migrate
set GOBIN=
```
...
```bash
go run github.com/imrany/gemmie/gemmie-server/cmd/migrate --command=version
```
...
./bin/migrate --command=goto --target=3
...
./bin/migrate --command=force --version=2
...
./bin/migrate --command=force --version=X
# 4. Re-run migrations
./bin/migrate --command=up
```
